...
/* GET home page. */
router.get('/api', function(req, res, next) {//注册一个路由，路由的path是"/"，
  res.render('index', { title: 'ManyTime' });//res.render表示渲染views->index.jade模版，并给这个模版传递一个值为ManyTime的title
});
...
router.post("/api/register",function(req,res){
	//获取请求参数数据
	const { username, password, type } = req.body; //从客户端发送过来的请求当中解构出这三个参数；
	//处理请求
		//判断用户是否已经存在，如果存在，返回错误提示信息；如果不存在，保存信息。
		//通过userModel查询(根据username)，

		UserModel.findOne({username}, function(err, user){ //此处的user是我们根据username，在数据库中找到的匹配文档
			if (user) {
				res.send({code: 1, msg: "此用户已存在！"+user});//用户存在则通过res返回响应
			}else{
				//password是解构过来的数据，因此需要写成键值对的形式：password:md5(password)
				UserModel({username, password:md5(password), type}).save(function(err,user){//在models.js文件中，UserModel的schema结构数据模型规定了username、password、type是必填项。
					//如果根据username没有找到user信息，那么就新建一个用户，新建用户的数据结构是UserModel中schema定义的数据结构，
					//新建的user数据是mongodb数据库当中，名为user的文档的其中一项数据。这里返回的是包含user的json格式数据
					//响应数据中不要携带密码
					const data = { username, type, id: user._id };
					console.log(username);
					//生成cookie(userid: user._id)来交给浏览器保存，保存时间为一天
...
// 	const { username, password, type } = req.body; //因为是用post方法发送的请求，此处用ES6的对象结构语法，获取三个前端传递过来的参数
// 	UserModel.findOne({username},function(err,user){
// 		if (user) {
// 			res.send({code:1, msg:"此用户已存在！"});
// 		}else{
// 			UserModel({username, type, password: md5(password)}).save(function(err,user){
// 				const data = { username, type, id: user._id };
// 				res.cookies('userid', user._id, {maxAge: 1000*60*60*24});
// 				res.send({code:0, data})
// 			})
// 		}
// 	})
...
//由前台的redux/actions里的updateUser()函数发送过来的数据，而这个函数由laoban-info/dashen-info调用，传入的是信息完善界面的参数
router.post('/api/update',function(req,res){
	//得到提交的用户数据
	//前面我们将user._id以userid的形式存储在浏览器里了。当发送请求的时候。浏览器会自动携带userid
	const userid = req.cookies.userid; //此处的userid是登陆/注册成功时保存在浏览器cookies里的{userid: user._id}
	//如果不存在，直接返回一个提示信息的结果
	if (!userid) {
		return res.send({code:1, msg: "请先登陆"})
	}
	//如果存在，则根据userid更新对应的user文档数据
	const user = req.body; //传过来的用户数据根据type决定，包括{header,post,info/company,salary[laoban有最后两个数据]},没有_id
...
router.get('/api/userlist',function(req,res){
	...
	UserModel.find({type}, filter, function(error, users){
		res.send({code: 0, data: users});//返回文档的集合。如果文档命名为user，那么对应的集合就是users。
	})
});
...
//修改指定消息为已读
router.post('/api/readmsg',function(req,res){ //需要修改数据库的数据。所有用post请求
	//得到请求中的from和to
